...
int DefaultSpriteSlot = SLOT_DEFAULT_SPRITES;
int DefaultSpriteID = 11;

void cbInitLevel(int LevelNow, int LevelOld, DWORD FIL_Flags)
{
	// Load global envmap settings
	for (int i = 0; i < MyData.BaseCustomizeMine.TotCustomize; i++)
	{
		if (MyData.BaseCustomizeMine.pVetCustomize[i].CustValue == CUST_ENVMAP_GLOBAL)
		{
			StrGenericCustomize* cust = &MyData.BaseCustomizeMine.pVetCustomize[i];
			
			DefaultSpriteSlot = cust->pVetArg[GlobalSpriteSlot];
			DefaultSpriteID = cust->pVetArg[GlobalSpriteID];
		}
	}
}
